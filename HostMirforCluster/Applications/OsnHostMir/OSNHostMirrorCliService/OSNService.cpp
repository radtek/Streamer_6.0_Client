...
{
    strcpy(m_szServiceName, "OSNCliService");
	strcpy(m_szServiceDisplayName, "EIM HostMirror Client Service");
...
BOOL COSNService::Install()
{
	...
	if (advapi32)
	{
		...
		{			
			...
			sd.lpDescription = __TEXT("OSNCliService.");				
    ...
    char szKey[256];
    HKEY hKey = NULL;
    strcpy(szKey, "SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\OSNCliService");
...
BOOL COSNService::Initialize()
{
	...
	stopServiceEventHandle = ::CreateEvent(NULL,FALSE,FALSE,"StopHostmirrorClientServiceEvent");
...
DWORD COSNService::StartThread()
{
	...
	LogMessage("INFO: HostMirrorClient Service Main thread started.\0");
	DWORD status;

	status = m_OSNRpcServer.StartSocketThread();
	if(status)
	{
		LogMessage("ERROR: Failed to start Server HostMirrorClient Linsten Socket.",status);
	}
	else 
	{
		LogMessage("INFO: OSN HostMirrorClient Linsten Socket started.\0.",status);
	}

	status = m_OSNRpcServer.StartListenThread();//创建Thread,发送和接受远程过来的信息；
	if (status)
		LogMessage("ERROR: Failed to start Server HostMirrorClient Listen Thread.",status);
	else
	...
	LogMessage("INFO: OSN HostMirror HostMirrorClient Service has been stopped.\n");
...
DWORD	COSNService::	SendSocketCommand(char *pBuffer,
													 ...
													 ULONG ipAddress)
	{
		...
		if(bufferSize>OSNRPC_HCMAX_MSG_LEN-OSNRPC_HCMSGHEAD_LEN)
		{
	   ...
	   char msg[OSNRPC_HCMAX_MSG_LEN];
	   memset(msg,0,sizeof(msg));

	   HC_MESSAGE_HEADER	messageHeader;
       memset(&messageHeader,0,sizeof(HC_MESSAGE_HEADER));


       memcpy(&msg[OSNRPC_HCMSGHEAD_LEN],pBuffer,bufferSize);
	   msg[OSNRPC_HCMSGHEAD_LEN+bufferSize]='\0';
	   ...
	   memcpy(msg,&messageHeader,sizeof(HC_MESSAGE_HEADER));
	   ...
	   if(connect(ConnectSocket,(struct sockaddr *)&targetSocket,sizeof(SOCKADDR_IN)))
	   {
		...
		if(!m_OSNRpcServer.OSNRpcSendMsg(targetSocket, 
										msg, 
										sizeof(HC_MESSAGE_HEADER) + messageHeader.dataLength,
										ConnectSocket))
		{
		...
		DWORD m_nError=recv(ConnectSocket,msg,OSNRPC_HCMAX_MSG_LEN,0);
		...
		memcpy(&messageHeader,msg,OSNRPC_HCMSGHEAD_LEN);
		memcpy(pBuffer,&msg[OSNRPC_HCMSGHEAD_LEN],bufferSize);
